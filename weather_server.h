...
void oled_outp_weather_daybig(){
  if(millis()-lastcheckweather>=checkweatherduration*1000||(weather_des==""&&weather_fetch!=0)){
    getweather();
  }
  ...
  if(weather_fetch==1){
    ...
    oled1.del_data();
    oled1.add_data("Can't access weather information... Network error",u8g2_font_6x12_tf,0,12);
  }
